...
export async function execute(interaction) {
  ...
  switch (action) {
    case 'embed':
      ...
      const fetchRole1 = guild.roles.cache.find(r => r.name === 'Embed restricted' && r.permissions.bitfield === 0n);
      ...
      await member.roles.add(fetchRole1).catch(console.error);
    ...
    case 'emoji':
      ...
      await wait(3000);
      // had to fetch role again to avoid old results.
      const fetchRole2 = guild.roles.cache.find(r => r.name === 'React restricted' && r.permissions.bitfield === 0n);
      ...
      await member.roles.add(fetchRole2).catch(console.error);
    ...
    case 'react':
      ...
      await wait(3000);
      // had to fetch role again to avoid old results.
      const fetchRole = guild.roles.cache.find(r => r.name === 'React restricted' && r.permissions.bitfield === 0n);
      ...
      await member.roles.add(fetchRole).catch(console.error);
